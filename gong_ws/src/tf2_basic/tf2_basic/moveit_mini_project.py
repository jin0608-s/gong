...
class MoveItMiniProjectNode(Node):
    def __init__(self):
        super().__init__("moveit_mini_project_node")
        ...
        # 💡 [360도 승하강 연속 회전 시퀀스]
        # 시작(init) ➡️ 1번 위 ➡️ 1번 아래(내려가기) ➡️ 1번 위(올라오기) ➡️ 2번 위(회전) ➡️ 2번 아래 ... 최종(init)
        self.pose_sequence = [
            "init",           # [처음] 초기 위치 대기
            
            "pose1_up",       # 1번 구역 상공으로 이동
            "pose1_down",     # 1번 사잇길로 내려가기
            "pose1_up",       # 다시 위로 올라오기
            
            "pose2_up",       # 올라온 상태에서 2번 구역으로 공중 회전
            "pose2_down",     # 2번 사잇길로 내려가기
            "pose2_up",       # 다시 위로 올라오기
            
            "pose3_up",       # 올라온 상태에서 3번 구역으로 공중 회전
            "pose3_down",     # 3번 사잇길로 내려가기
            "pose3_up",       # 다시 위로 올라오기
            
            "pose4_up",       # 올라온 상태에서 4번 구역으로 공중 회전
            "pose4_down",     # 4번 사잇길로 내려가기
            "pose4_up",       # 다시 위로 올라오기
            
            "pose5_up",       # 올라온 상태에서 5번 구역으로 공중 회전
            "pose5_down",     # 5번 사잇길로 내려가기
            "pose5_up",       # 다시 위로 올라오기
            
            "pose6_up",       # 올라온 상태에서 6번 구역으로 공중 회전
            "pose6_down",     # 6번 사잇길로 내려가기
            "pose6_up",       # 다시 위로 올라오기
            
            "init"            # [끝] 모든 순회 완료 후 최종 복귀
        ]
        self.current_step = 0
        
        time.sleep(1.5) # 환경 동기화 대기

        # 3.5초마다 한 단계씩 끊임없이 진행하는 타이머 루프
        self.timer = self.create_timer(3.5, self.timer_callback)
        self.get_logger().info("사잇길 승하강 방식의 360도 연속 순회 제어기가 시작되었습니다.")

    def create_collision_object(self, obj_id: str, dimensions: list, position: list, orientation: list = [0.0, 0.0, 0.0, 1.0]):
        collision_object = CollisionObject()
        collision_object.header.frame_id = "world"
        collision_object.header.stamp = self.get_clock().now().to_msg()
        ...
        return self.planning_scene_monitor.process_collision_object(collision_object)

    def setup_environment(self):
        # 베이스 충돌 제약 방지용 하향 조정한 작업 테이블
        self.create_collision_object("base_table", [1.2, 1.2, 0.05], [0.0, 0.0, -0.06])

        # 반지름 0.42m 거리에 60도 간격 방사형 장벽 6개 배치
        num_walls = 6
        radius = 0.42
        wall_dimensions = [0.15, 0.02, 0.25]
        ...
        for i in range(num_walls):
            ...
            self.create_collision_object(
                obj_id=f"radial_wall_{i}", 
                dimensions=wall_dimensions, 
                position=[x, y, z], 
                orientation=orientation
            )
        self.get_logger().info(f"360도 원형 장벽 {num_walls}개가 정상 배치되었습니다.")

    def plan_and_execute(self, configuration_name: str, controller_name: str = "arm_controller") -> bool:
        self.arm.set_start_state_to_current_state()
        self.arm.set_goal_state(configuration_name=configuration_name)
        ...
        if plan_result:
            ...
            self.get_logger().info(f"▶동작 완료 포즈: [{configuration_name}]")
            return True
        else:
            self.get_logger().error(f"❌경로 차단됨(충돌 위험): [{configuration_name}]")
            return False

    def timer_callback(self):
        target_pose = self.pose_sequence[self.current_step]
        self.get_logger().info(f"시퀀스 {self.current_step + 1}/{len(self.pose_sequence)} 실행: {target_pose}")
        
        self.plan_and_execute(target_pose)

        # 무한 반복 순회 계산
        self.current_step = (self.current_step + 1) % len(self.pose_sequence)
